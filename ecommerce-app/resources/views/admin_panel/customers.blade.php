...
@section('content')
  ...
  <div class="col-12 mx-auto bgc-white rounded-3 mt-4 mx-4" style="min-height: 70vh;">
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between px-5">
      <h2 class="txt-primary font-reg">Customers</h2>
    ...
    <div class="col-11 rounded-3  mx-auto" style="min-height: 10vh;">
        ...
        <table class="table bgc-primary rounded-3 my-3" style="min-height: 15vh; border:transparent">
        <thead>
          <tr>
            <th style="font-size: 18px;" class="txt-light font-med px-4">ID</th>
            <th style="font-size: 18px;" class="txt-light font-med px-4">Name</th>
            <th style="font-size: 18px;" class="txt-light font-med px-2">Email</th>
            <th style="font-size: 18px;" class="txt-light font-med px-2">Phone</th>
            <th style="font-size: 18px;" class="txt-light font-med px-2">Action</th>
          </tr>
        </thead>
        <tbody class="col-12 bgc-light rounded-3 mt-3">
          ...
          <tr>
            <td style="font-size: 18px;" class="font-reg px-4">{{$customer->id}}</td>
            <td style="font-size: 18px;" class="font-reg px-4">{{$customer->name}}</td>
            <td style="font-size: 18px;" class="font-reg px-2">{{$customer->email}}</td>
            <td style="font-size: 18px;" class="font-reg px-2">{{$customer->phone_num}}</td>
            <td style="font-size: 18px;" class="font-reg px-2 text-center d-flex align-items-center">
              <a href="{{route('orders_details', ['id' => $customer->id])}}">
                <i class="fas fa-eye pt-2" style="font-size: 16px;"></i>
              </a>
              <form action="{{ route('users.delete_user', ['id' => $customer->id]) }}" method="POST">
                @csrf
                @method('DELETE')
